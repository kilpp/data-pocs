...
- [ ] Set up development environment (Scala/Java)
- [ ] Set up monitoring stack (Prometheus + Grafana)

### Phase 2: Ingestion (Week 2)
- [ ] Implement JDBC Source Connector for relational DB
- [ ] Implement File Source Connector
- [ ] Build custom SOAP connector
- [ ] Test data flow into Kafka topics

### Phase 3: Processing Pipelines (Week 3-4)
- [ ] Implement Top Sales by City pipeline
- [ ] Implement Top Salesman pipeline
- [ ] Add data validation and quality checks
- [ ] Implement error handling and dead letter queues

### Phase 4: Lineage & Observability (Week 5)
- [ ] Integrate OpenLineage with Spark jobs
- [ ] Set up Marquez for lineage visualization
...
kafka-topics --create --topic top-sales-city --bootstrap-server localhost:9092
kafka-topics --create --topic top-salesmen --bootstrap-server localhost:9092
...
# 6. Submit Spark job
spark-submit \
  --class com.company.pipelines.Main \
  --master local[*] \
  --packages org.apache.spark:spark-sql-kafka-0-10_2.13:3.5.0 \
  target/scala-2.13/sales-pipeline_2.13-1.0.jar
...
```scala
// Using ScalaTest
class TopSalesByCitySpec extends AnyFlatSpec with Matchers {
  "TopSalesByCity" should "aggregate sales correctly" in {
    val spark = SparkSession.builder()
      .master("local[*]")
      .getOrCreate()
    ...
    val testData = Seq(
      Sale("NYC", "John", 100.0, Timestamp.valueOf("2026-01-23 10:00:00")),
      Sale("NYC", "Jane", 150.0, Timestamp.valueOf("2026-01-23 11:00:00")),
      Sale("LA", "Bob", 200.0, Timestamp.valueOf("2026-01-23 12:00:00"))
    ).toDF()
    
    val result = TopSalesByCity.aggregate(testData)
    
    result.count() shouldBe 2
    result.filter($"city" === "NYC").select("total_sales").as[Double].head shouldBe 250.0
  }
}
```
...
### Integration Tests
```scala
class PipelineIntegrationSpec extends AnyFlatSpec {
  "End-to-end pipeline" should "process data from Kafka to PostgreSQL" in {
    // 1. Publish test data to Kafka
    // 2. Run Spark job
